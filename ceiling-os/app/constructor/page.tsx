...
const initialDrawing: CeilingDrawing = { id: "draft-room", objectId: "demo-object", version: 1, createdAt: now, updatedAt: now, walls: [
...
const initialObject: OfflineObject = { id: "demo-object", companyId: "demo-company", address: "Новий об'єкт", status: "measurement", updatedAt: now };
...
export default function ConstructorPage() {
  ...
  const [history, setHistory] = useState<CeilingDrawing[]>([]);
  const [future, setFuture] = useState<CeilingDrawing[]>([]);
  ...
  function applyDrawing(next: CeilingDrawing) {
    setHistory((items) => [...items.slice(-29), clone(drawing)]);
    setFuture([]);
    setDrawing(next);
  }
  function undo() {
    ...
    setFuture((items) => [clone(drawing), ...items.slice(0, 29)]);
    setDrawing(previous);
  }
  function redo() {
    const next = future[0];
  ...
  function updateSelectedWallLength(value: string) {
    ...
    const next = { x: selectedWall.start.x + (dx / current) * length, y: selectedWall.start.y + (dy / current) * length };
    applyDrawing({ ...drawing, walls: drawing.walls.map((wall) => wall.id === selectedWall.id ? { ...wall, end: next } : wall), version: drawing.version + 1, updatedAt: new Date().toISOString() });
  }
  async function handleSave() {
    try { await saveOfflineDrawing(initialObject, drawing); setSaved(true); window.setTimeout(() => setSaved(false), 1800); } catch { setSaved(false); }
  }
  ...
  return <main className="constructor-page">
    <header className="constructor-header">
      ...
      <div className="constructor-actions"><button type="button" onClick={undo} disabled={!history.length}>↶ Назад</button><button type="button" onClick={redo} disabled={!future.length}>↷ Вперед</button><button className="save-button" type="button" onClick={handleSave}>{saved ? "Збережено ✓" : "Зберегти"}</button></div>
    </header>
    <ConstructorToolbar tool={tool} onToolChange={setTool} />
    <section className="constructor-layout">
      <div className="canvas-card"><CeilingCanvas drawing={drawing} tool={tool} onChange={applyDrawing} onSelectWall={setSelectedWallId} /></div>
      <aside className="calc-card">
        ...
        <div className="metric"><span>Кріплення</span><strong>{result.fasteners}</strong></div>
        <div className="metric"><span>Світильники</span><strong>{result.lights}</strong></div>
        {selectedWall && <div className="wall-editor"><h3>Вибрана стіна</h3><label>Довжина, м<input type="number" min="0.1" step="0.01" value={distance(selectedWall.start, selectedWall.end).toFixed(2)} onChange={(event) => updateSelectedWallLength(event.target.value)} /></label><div className="wall-angle">Кут: {wallAngleDeg(selectedWall).toFixed(1)}°</div></div>}
