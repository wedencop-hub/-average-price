...
export function calculateInternalCost(objectId:string, items:CostItem[], revenue:number, installationCost=0, deliveryCost=0, otherCost=0):InternalCost{
 const materialCost=round(items.reduce((sum,item)=>sum+item.total,0));
 const totalCost=round(materialCost+installationCost+deliveryCost+otherCost);
 const profit=round(revenue-totalCost);
 return {objectId,items,materialCost,installationCost,deliveryCost,otherCost,totalCost,revenue,profit,margin:revenue>0?round(profit/revenue*100):0};
}
